...
function renderAccounts(){
  ...
  summary.innerHTML = `
    ...
    <div class="acc-break-container">
      <div class="acc-assets" onclick="toggleAccSummaryTag(this)">
        <div class="acc-break-tag">
          <span class="label">總資產</span>
          <span class="amt" style="color:var(--blue)">${fmt(posTotal)}</span>
        </div>
      </div>
      <div class="acc-liab" onclick="toggleAccSummaryTag(this)">
        <div class="acc-break-tag">
...
async function renderHome(){
  ...
  renderMonthCalendar();
  renderDateHeader();
  renderDaySummary(S.selDate); // 初始化今日收支摘要
  ...
  for(let r=0;r<6;r++){
    ...
    for(let c=0;c<7;c++){
      ...
      if(r===0 && c<startDay){cell.textContent='';}
        ...
        cell.dataset.ds = ds; // 儲存日期字串，供 updateCalendarCells 快速存取
        cell.addEventListener('click',(e)=>{
          e.stopPropagation(); // 防止事件冒泡造成重複觸發
          S.selDate=ds;
          ...
          // 只更新 CSS 選中狀態，不重新渲染整個日曆格，避免畫面跳動
          updateCalendarCells();
          renderDateHeader();
          renderDaySummary(ds); // 更新今日收支摘要
          renderDayRecs(ds);
        });
        day++;
      }
...
/* ── 只更新日曆格的 CSS 選中狀態，不重建 DOM，避免點擊跳動 ── */
function updateCalendarCells(){
  const today = todayStr();
  document.querySelectorAll('#month-grid .month-cell[data-ds]').forEach(cell=>{
    const ds = cell.dataset.ds;
    cell.classList.toggle('today', ds === today); // 今日樣式
    cell.classList.toggle('sel',   ds === S.selDate); // 選中樣式
  });
}

/* ── 今日收支摘要：顯示選取日期的支出與收入小計 ── */
function renderDaySummary(ds){
  const el = document.getElementById('day-summary');
  if(!el) return;
  // 篩選選取日期的記錄
  const recs = S.monthRecs.filter(r => r.date === ds);
  const expense = recs.filter(r => r.type === 'expense').reduce((s,r) => s + (parseFloat(r.amount)||0), 0);
  const income  = recs.filter(r => r.type === 'income' ).reduce((s,r) => s + (parseFloat(r.amount)||0), 0);
  // 若當日無任何支出或收入，隱藏摘要列
  if(!expense && !income){ el.innerHTML=''; el.style.display='none'; return; }
...
async function renderReport(){
  ...
  function renderCatReport(drillParent){
    ...
    ce2.innerHTML='<div class="cat-toggle"><button class="cat-toggle-btn'+(S.rptCatType==='expense'?' on':'')+'" data-type="expense">支出</button><button class="cat-toggle-btn'+(S.rptCatType==='income'?' on':'')+'" data-type="income">收入</button></div>'
      // 容器放大至 300px，overflow:visible 確保外圍標籤不被裁切
      +'<div style="position:relative;width:300px;height:300px;margin:0 auto 14px;overflow:visible"><canvas id="pie-chart"></canvas>'
      // 中心顯示總金額
    ...
    S.pieChart=new Chart(cx,{type:'doughnut',data:{labels:vl.length?lb:['無資料'],datasets:[{data:vl.length?vl:[1],backgroundColor:vl.length?cl:['#e0e0e0'],borderWidth:vl.length?2:0,borderColor:'#fff'}]},
      options:{responsive:true,maintainAspectRatio:false,
        // padding 加大，確保外圍數字標籤不被容器邊緣裁切
        layout:{padding:{top:56,bottom:56,left:72,right:72}},cutout:'60%',
...
document.getElementById('sub-add').addEventListener('click',async()=>{
  ...
  // 商家、專案、帳戶分組 → 開啟美觀的簡易新增 overlay（取代 prompt）
  if(S.subPageKey==='merchants'||S.subPageKey==='experts'||S.subPageKey==='account_groups'){
    openSimpleAddOverlay(S.subPageKey);
    return;
  }
...
/* ══════════════════════════════════════════════
   簡易新增 Overlay（商家、專案、帳戶分組）
══════════════════════════════════════════════ */
let _simpleAddKey = ''; // 目前要新增的設定類型

/* 開啟簡易新增 overlay，依類型顯示對應標題 */
function openSimpleAddOverlay(key){
  _simpleAddKey = key;
  const titles = {
    merchants:      '新增商家',
    experts:        '新增專案',
    account_groups: '新增帳戶分組',
  };
  document.getElementById('add-simple-title').textContent = titles[key] || '新增';
  document.getElementById('add-simple-name').value = '';
  document.getElementById('add-simple-overlay').classList.add('show');
  setTimeout(() => document.getElementById('add-simple-name').focus(), 200);
}

/* 關閉簡易 overlay */
document.getElementById('add-simple-cancel').addEventListener('click', () => {
  document.getElementById('add-simple-overlay').classList.remove('show');
});

/* 確認新增 */
document.getElementById('add-simple-ok').addEventListener('click', async () => {
  const name = document.getElementById('add-simple-name').value.trim();
  if(!name){ toast('請輸入名稱'); return; }
  await api(`/api/settings/${_simpleAddKey}`, {method:'POST', body:JSON.stringify({name})});
  document.getElementById('add-simple-overlay').classList.remove('show');
  await loadCfg();
  renderSubList();
  // 帳戶分組新增後也更新帳戶總覽頁
...
/* Enter 鍵快速確認 */
document.getElementById('add-simple-name').addEventListener('keydown', e => {
  if(e.key === 'Enter') document.getElementById('add-simple-ok').click();
});

/* 點擊背景關閉 */
document.getElementById('add-simple-overlay').addEventListener('click', e => {
  if(e.target === document.getElementById('add-simple-overlay'))
    document.getElementById('add-simple-overlay').classList.remove('show');
});
...
async function renderTrendChart(mode){
  ...
  // 每日至少 28px 寬，確保日期可讀；最小 300px
  const chartMinWidth = Math.max(labels.length * 28, 300);
  el.innerHTML=
    ...
    +'</div></div>'
    // 外層：水平捲動容器；內層：固定最小寬度讓日期不擠壓
    +'<div class="trend-scroll-wrap">'
    +'<div style="position:relative;height:220px;min-width:'+chartMinWidth+'px;">'
  ...
  S.trendChart=new Chart(ctx,{type:'line',
    ...
    options:{
      // responsive:false 搭配 CSS min-width，讓圖表在捲動容器中保持完整尺寸

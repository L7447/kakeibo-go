...
function renderAccounts(){
  ...
  const globalTotal = posTotal - negTotal;
  ...
  summary.innerHTML = `
    ...
    <div class="acc-summary-break">
      ...
      <span class="liab">總負債 ${fmt(negTotal)}</span>
    </div>`;
...
function openAddPage(rec){
  ...
  const _standaloneExp2 = document.getElementById('standalone-exp');
  if(_standaloneExp2) _standaloneExp2.style.display = (S.addType==='expense') ? '' : 'none';
...
document.querySelectorAll('.type-tab[data-t]').forEach(t=>t.addEventListener('click',()=>{
  ...
  if(S.addType==='transfer'){
    ...
    document.getElementById('standalone-exp').style.display='none';
  } else if(S.addType==='adjust'){
    ...
    infoSecEl.style.display='';
    totalDispEl.style.display='';
    document.getElementById('standalone-exp').style.display='';
  }
  document.getElementById('cat-section').style.display='none';
  renderAddItems(); calcTotal();
}));
...
/* ══════════════════════════════════════════════
   進階功能：存錢筒 / 預算 / 固定支出收入
══════════════════════════════════════════════ */
const ADV_KEYS={piggy:'adv_piggy',budget:'adv_budget',recurring:'adv_recurring'};
...
function saveRecurring(){
  ...
  const items=advLoad('recurring');
  items.push({type,category,amount,startdate,time,periodN,periodUnit,note,lastRun:''});
